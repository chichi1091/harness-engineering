...
- Execution Visualization（Issue #36、`src/run/execution-visualization.js`）を導入。Execution History（#35）の**投影（Projection）**として、`buildExecutionVisualization` がHistoryデータから実行フローのタイムライン（Goal→Plan→Step attempt→Verification→Fallback→PR Automation→Pull Request）を構築し、`formatExecutionVisualization` が人間可読テキストへ変換する。Retry（`↻` 同一Stepの再実行）とFallback（provider/model切替）を区別表示し、Escalationは記録されたデータが存在する場合のみ表示（推測しない）。pr-automation artifact（#37）はBranch/Commit/PR URLとしてFlowに表示。CLIは `harness history <id> --timeline`、表示用の状態・新規データ記録は持たない（#29/#35のSchema変更なし、ANSI非依存）。8件のテストを追加
- PR Automation（Issue #37、`src/automation/`）を導入。`decidePullRequestAutomation` が Mechanical Verification（#28）の結果を唯一の品質ゲートとして機械判定し、completed+verification passed+unresolvedゼロ+必須artifact揃いの場合のみGit操作とPR作成に進む。Git操作はGuarded Command Runner（#27）経由で、commit/pushを `enforceAction`（git operation）で検査し、force push等の破壊的操作は既存destructive検出で拒否（**自動mergeの機能はPortに存在しない**）。PR本文はExecution Result / Verification / Model Usage(#22) / Unresolvedから `buildPullRequestBody` が構築し、`redactSecrets` で認証情報を除外。結果は `pr-automation` artifact（新規登録の共通Schema型）としてHistoryへ記録（skipped/failed/created/dry-runと理由を保持）。`harness run --create-pr`（`--pr-dry-run` / `--pr-base` 対応）でCLI統合。単体13件を追加
- Issue → Harness（Issue #38、`src/issues/`）を導入。GitHub Issueを構造化したHarnessInputへ変換し、`harness run --issue <n> --repo owner/repo` / `--issue-url <url>` / `harness plan --issue …` の入口でPlan/Run/Execution/Historyへ接続する。Issue Resolver Port（Core抽象）の実装はGitHub Adapter（`gh` CLI経由、`src/issues/github-issue-adapter.js`）とMock（組み込みfixture、`src/issues/mock-issue-adapter.js`）で、CoreはGitHubを知らない。`resolveIssueInput` が goal（=Issue title）/source（type/repository/issueNumber/url/author）/context（labels/author/state/**untrusted envelope**）を構築し、labelsはintentの入力ヒントとしてDecision Engineへ渡される（Workflow選択は常に既存Engine）。Issue本文は `wrapUntrusted` で境界内に包まれ、boundary markerを含む本文は取り込み拒否（境界偽装防止）— 本文の命令がGuardrails/権限/Policyを変更する経路は存在しない。closed issueは既定で拒否（`--allow-closed-issue`で明示許可）。取得失敗は not_found/auth_error/unavailable に機械分類され入力段階の失敗として扱う（Developer Step失敗とは区別）。`source` はExecution Result/Historyへ記録され#37の突合キーになる。Resolver単体8件+統合4件+CLI E2E 8件を追加
- Skills / Lazy Loading（Issue #30、`src/skills/skill-registry.js` + `skills/`）を導入。Agent(誰が)/Workflow(順序)と分離された「専門手順」として、`skills/<skill-id>/skill.yaml`（metadata: id/version/capabilities/triggers/appliesTo/procedure）と `SKILL.md`（本文）を定義。Registry(`createSkillRegistryFromDirectory`)は**metadataのみ**を構築時に読み、本文は選択後の `loadSkillContent(id)` でのみロードされる**Lazy Loading**（未使用SkillはContextに含まれない — テストで保証）。選択(`selectSkillsForStep`)は保守的: 1件=selected、0件=none、複数=ambiguous（勝手に選ばず明示選択を要求）。`validateSkillMetadata` が語彙検証し不正SkillはRegistry外。SKILL.mdにuntrusted boundary markerが含まれる場合はロード拒否（境界偽装防止）。ロードされたSkillは `request.skills`(#31契約拡張)でRuntime promptへ注入され、StepRecordに `skills`/`skillsAmbiguous` として記録（Plan上のplanned skillと実行時にロードされた `skill_id@version` を区別）。Plan(#34)表示にSkills節を追加。正本Skillとして `unit-test-design` と `security-review` を追加。Skillは権限を持たず、危険手順もGuardrails(#27)で拒否されることを統合テストで実証。Registry単体8件+統合6件を追加
- Execution History（Issue #35、`src/run/execution-history.js` + `bin/harness.js` の `history` サブコマンド）を導入。Artifact Store（#29）に記録済みのExecution Result / Execution Plan / Model Execution Record / Verification Resultを `execution_id` で集約する**Read Model**であり、新しいDBや二重保存を持たない。一覧（`harness history`、新着順、`--limit` / `--status` / `--workflow` / `--since` フィルタ、各executionの `execution-result` 決定的パスのみ読む高速サマリ）と詳細（`harness history <id>`、Plan/Steps(attempt・retry・fallback)/Model Records/Verification gates/Guardrail拒否/Artifact目録をID参照）を提供し、`--json` で機械可読出力。存在しないexecution idはexit 1。`redactSecrets`（#27/#22と同一検出器）によりfailure reasonから認証情報を除外。`harness run` は実行完了時に `execution-result` artifact（新規登録の共通Schema型）を既定 `.harness/artifacts/` へ保存し、Store primitiveには `listExecutionIds` / `readExecution` / `readExecutionResult` を最小追加。History単体14件+CLI E2E 5件を追加
- harness plan CLI（Issue #34、`src/run/execution-plan.js` + `bin/harness.js` の `plan` サブコマンド）を導入。Goal → Decision Engine → Execution Plan の生成までを**副作用ゼロ**で実行する（node:fs/child_process不使用を機械検査、`--output` 指定時のみplanファイルを書込）。Planには task（goal/intent/risk）、workflow、steps（stepId/role/gate/tokenBudget/dependencies）、models（**planned** 承認前の予定 — Profile Tier解決結果、実行時の正本は#22 Tracking）、runtime、tokenBudget、retryPolicies、fallbackPolicy、guardrailsSummary、verification を含む。**Plan Integrity**: `planHash` は実行構成部分のcanonical SHA-256で、`verifyPlanIntegrity` が承認後の改変を検出する（bookkeeping項目はhash対象外のため承認してもhash不変）。`harness run --plan <file>` は承認済みPlanのhash検証後に**同一Plan**を実行し（Plan駆動ではDecision Engineを再実行しない）、未承認はexit 3、改変は `Plan改変を検出` で拒否。`execution-plan` artifact型（#29 Store保存可能）を新規登録。`--json` 機械可読出力と `--output` 保存に対応。Plan単体7件+CLI E2E 9件を追加
- harness run CLI（Issue #33、`bin/harness.js` + `src/run/run-harness.js`）を導入。目的の自然文1行から Decision EngineによるWorkflow選択 → Execution Engine実行 → Runtime Adapter（OpenCode #32 / Mock）→ Guardrails(#27) → Mechanical Verification(#28) → Retry/Fallback(#23) → Artifact Store(#29)・Model Execution Tracking(#22)への永続化までを1本の実行フローとして接続。CLIはCompositionとI/Oのみを担い、実行ロジックは既存コンポーネントに委譲（Decision/Executionの新規ロジックなし）。承認済みPlan（`approved: true`必須、#34との最小契約）の実行、非対話モード、機械判定可能な終了コード（0成功/1失敗停止/2入力不正/3未承認）、人間可読レンダリング（Steps/Verification/Model Usage/Retry/Fallback/Result）を提供。runHarnessはexecuteStep/registry/storeを注入式で受け取るためMock Runtimeで完全テスト可能。CLI E2E 8件+Composition 11件を追加
- Fallback Policy（Issue #23、`src/execution/fallback-policy.js` + `src/runtimes/fallback-runtime-adapter.js`）を導入。Provider / Modelが一時的に利用不能になった場合、Policy宣言した優先順位付き候補へ切り替えて同一Stepを再実行する。**Runtime Adapter層のDecorator**として実装し、Execution Engineは無変更（Fallbackは内部試行、attemptはEngineが数えるStep実行回数のまま）。対象エラーは `FALLBACK_ELIGIBLE_ERROR_CATEGORIES`（#31語彙を単一源として共有: rate_limited / quota_exceeded / provider_unavailable / timeout / transient_error）のみで、認証エラー・不正Model/設定・Permission error・Guardrails違反・未分類は候補が残っていても即返る（**Guardrails違反をFallbackで回避する経路は存在しない**）。候補列は有限・重複なし（宣言検証で拒否）・前方消費のみで、`maxFallbacks` で切り詰められるため循環（A→B→A）は構造的に不可能。全候補試行は `outcome.runtime` の `fallbackChain` / `fallbackCount` / `fallbackReason` / `fallback`（#22の予約形状を拡張: count/attempts付き）として報告され、Model Execution Recordへ自動転記される。Guardrailsとの関係: delegate生成に同一のGuarded Command Runner構成を注入するためProvider切替でGuardrailsは弱まらない。単体9件+統合5件を追加
